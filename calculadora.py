...
class CalculatorApp(App):
    def build(self):
        ...
        for button in buttons:
            is_operator = button in self.operators or button in ['C', '=', 'CE', '<-']
            btn = CustomButton(
                text=button,
    ...
    def on_button_press(self, instance):
        ...
        if button_text == 'C' or button_text == 'CE':
            ...
            try:
                num = int(eval(current))
                if num < 0:
                    raise ValueError
                self.input_box.text = str(math.factorial(num))
